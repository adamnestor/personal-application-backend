...
public class BudgetService {
    ...
    private BigDecimal getStartingBalanceForMonth(int year, int month) {
        ...
        boolean hasDataBefore = scheduledExpenseRepository.findByYearAndMonthValueOrderByScheduledDate(
                previousMonth.getYear(), previousMonth.getMonthValue()).size() > 0 ||
                scheduledIncomeRepository.findByYearAndMonthValueOrderByScheduledDate(
                        previousMonth.getYear(), previousMonth.getMonthValue()).size() > 0;
